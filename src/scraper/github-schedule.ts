...
import { createHeader, createSectionHeader } from '../utils/console-utils.js';
...
createHeader('GitHub API Time Distribution & Rate Limit Analyzer');
console.log();
...
createSectionHeader('Rate Limit Distribution');
...
createSectionHeader('Search Strategy');
const searchQueries = [
  'free AI models',
  'open source LLM',
  'free API providers',
...
createSectionHeader('Fresh Content Strategy');
...
createSectionHeader('Example Scrape Schedule');
const now = new Date();
...
createSectionHeader('Optimization Benefits');
...
if (env.GITHUB_TOKEN) {
  createSectionHeader('Credentials Detected');
